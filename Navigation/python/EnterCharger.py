...
class EnterExitCharger:
    ...
    def step_enter_charger(self, t:float):
        ...
        if (t < self.t_turn): 
            ...
            return self.p5x, self.p5y, self.theta1, 0 #holding
    ...
    def step_exit_charger(self, t:float):
        if (t < self.t_runup):
            ...
            distance = self.accel_profile.x(t - self.t_turn - self.t_linear, self.radial_distance)
            change_x =  self.p4x + self.r*(1-np.cos(distance/self.r))
            change_y = self.p4y + self.r*np.sin(distance/self.r)
            return change_x, change_y, self.theta_prime, 1
...
if __name__ == '__main__':
    ...
    enter_exit = EnterExitCharger(0, 1.5, np.pi/4, 1.5, 0, np.pi/4, 0.5, 0.25, 0.3, 0.5, 4, 8)
    tf = enter_exit.total_time()
    n = 10000
    ...
    for i in range(n):
        x,y,theta, flag = enter_exit.step_enter_charger(ts[i])
        xs[i] = x
        ys[i] = y
        ths[i] = theta
